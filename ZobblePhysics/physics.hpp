...
public:
    ...
    void solveContact(uint32_t atom_1_idx, uint32_t atom_2_idx)
    {
        ...
        if (dist2 < 1.0f && dist2 > eps) {
            ...
            const Vec2 col_vec = Vec2(o2_o1.x / dist * delta, o2_o1.y / dist * delta);
            obj_1.setPosition(obj_1.position + col_vec);
            obj_2.setPosition(obj_2.position - col_vec);
        }
    }
    ...
    // Add a new object to the solver
    uint64_t createObject(Vec2 pos, simd::uchar4 col)
    {
        return objects.emplace_back(pos, col);
    }
    ...
    void updateObjects_multi(float dt)
    {
        thread_pool.dispatch(to<uint32_t>(objects.size()), [&](uint32_t start, uint32_t end){
            for (uint32_t i{start}; i < end; ++i) {
                ...
                // Add gravity
                obj.acceleration = obj.acceleration + gravity;
