...
public extension DecelerationTimingParameters {
    private var k: CGFloat { 1000 }
    
    var destination: CGPoint {
        let dCoeff = k * log(decelerationRate)
    ...
    func value(at time: TimeInterval) -> CGPoint {
        let dCoeff = k * log(decelerationRate)
        return initialValue + (pow(decelerationRate, CGFloat(k * time)) - 1) / dCoeff * initialVelocity
    }
    ...
    func velocity(at time: TimeInterval) -> CGPoint {
        return initialVelocity * pow(decelerationRate, CGFloat(k * time))
    }
